...
Vector.prototype = {
  ...
  cross: function (a) {
    ...
    var A = this.elements;
    return Vector.create([(A[1] * B[2]) - (A[2] * B[1]),
    (A[2] * B[0]) - (A[0] * B[2]),
    (A[0] * B[1]) - (A[1] * B[0])])
  },
  max: function () {
  ...
  rotate: function (t, a) {
    ...
    switch (this.elements.length) {
      case 2:
        ...
        return Vector.create([V[0] + R[0][0] * x + R[0][1] * y,
        V[1] + R[1][0] * x + R[1][1] * y]);
        break;
      case 3:
        if (!a.direction) {
        ...
        return Vector.create([C[0] + R[0][0] * x + R[0][1] * y + R[0][2] * z,
  ...
  reflectionIn: function (a) {
    if (a.anchor) {
      ...
      return Vector.create([C[0] + (C[0] - P[0]),
      C[1] + (C[1] - P[1]),
      C[2] + (C[2] - (P[2] || 0))])
    } else {
      var Q = a.elements || a;
...
Vector.i = Vector.create([1,
0,
0]);
Vector.j = Vector.create([0,
1,
0]);
Vector.k = Vector.create([0,
0,
1]);
...
Matrix.Rotation = function (b, a) {
  if (!a) {
    return Matrix.create([[Math.cos(b),
    - Math.sin(b)],
    [
      Math.sin(b),
      Math.cos(b)
...
Line.prototype = {
  ...
  translate: function (a) {
    var V = a.elements || a;
    return Line.create([this.anchor.elements[0] + V[0],
    this.anchor.elements[1] + V[1],
    this.anchor.elements[2] + (V[2] || 0)], this.direction)
  },
  isParallelTo: function (a) {
    if (a.normal) {
  ...
  pointClosestTo: function (a) {
    if (a.direction) {
      ...
      var V = Vector.create([D2 * x - D3 * z,
      D3 * y - b * x,
      b * z - D2 * y]);
      var k = this.distanceFrom(P) / V.modulus();
      return Vector.create([P[0] + V.elements[0] * k,
      P[1] + V.elements[1] * k,
  ...
  reflectionIn: function (a) {
    if (a.normal) {
      ...
      var P = a.elements || a;
      return Line.create(this.anchor.reflectionIn([P[0],
      P[1],
      (P[2] || 0)]), this.direction)
    }
  },
...
Plane.prototype = {
  ...
  translate: function (a) {
    var V = a.elements || a;
    return Plane.create([this.anchor.elements[0] + V[0],
    this.anchor.elements[1] + V[1],
    this.anchor.elements[2] + (V[2] || 0)], this.normal)
  },
  isParallelTo: function (a) {
    var b;
    if (a.normal) {
  ...
  intersectionWith: function (a) {
    ...
    if (a.direction) {
      ...
      return Vector.create([A[0] + D[0] * b,
      A[1] + D[1] * b,
      A[2] + D[2] * b])
    } else if (a.normal) {
  ...
  reflectionIn: function (a) {
    if (a.normal) {
      ...
      var P = a.elements || a;
      return Plane.create(this.anchor.reflectionIn([P[0],
      P[1],
      (P[2] || 0)]), this.normal)
    }
  },
  setVectors: function (a, b, c) {
    ...
    if (c !== null) {
      ...
      v22 = c.elements[1],
      v23 = c.elements[2];
      f = Vector.create([(v12 - A2) * (v23 - A3) - (v13 - A3) * (v22 - A2),
      (v13 - A3) * (g - d) - (e - d) * (v23 - A3),
      (e - d) * (v22 - A2) - (v12 - A2) * (g - d)]);
      mod = f.modulus();
      if (mod === 0) {
        return null
      }
      f = Vector.create([f.elements[0] / mod,
      f.elements[1] / mod,
      f.elements[2] / mod])
    } else {
      ...
      f = Vector.create([b.elements[0] / mod,
      b.elements[1] / mod,
      b.elements[2] / mod])
    }
    this.anchor = a;
